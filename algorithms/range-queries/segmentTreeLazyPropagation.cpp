...
struct node{
	...
	//update a range and propagate lazy
	void update(ll tl, ll tr, ll dif, ll l = 1, ll r = n){
		if (tl <= l && tr >= r){
			val += (r-l+1) * dif;
			lazy += dif;
		}
			...
			propagate(l,r);
			L->update(tl,tr,dif,l,MID);
			R->update(tl,tr,dif,MID+1,r);
			val = L->val + R->val;
		}
	}

	//query a range and propagate lazy
