//CF - EDU - SEG TREE PART 1 - STEP 4 - Problem D (Number of Different Segments): https://codeforces.com/edu/course/2/lesson/4/4/practice/contest/274684/problem/D
...
struct node{
	...
	void update(ll x, ll pos, ll l = 0, ll r = n-1){
		if (l == pos && l == r){
			val = x;
		}
		else if(l <= pos && r>= pos){
			...
			ll newL = l;
			ll newR = MID;
			if (newL <= pos && newR >= pos){
				if (L == nullptr) L = new node;
				L->update(x,pos,newL,newR);
				// val += L->val;
			}
	...
	ll query(ll tl, ll tr, ll l = 0, ll r = n-1){
		if (tl <= l && tr >= r){
			...
			ll ans = 0;
			if (L != nullptr){
				ans = L->query(tl,tr,l,MID);
			}
			if (!ans && R != nullptr){
				ans = R->query(tl,tr,MID+1,r);
			}
			return ans;
		}
	}
