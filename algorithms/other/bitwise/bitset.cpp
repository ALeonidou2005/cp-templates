...
int main(){
	...
	//creating a bitset of large size (using heap(dynnamic) memory instead of stack)
	vector<bitset<LARGE_N>> bs(1);
    auto &a = bs[0];
    dbg(a);

    //creating bitset of size 4
    bitset <4> b;
    dbg(b);

    //creating bitset of size 5, and initialise with decimal number
    bitset <8> c(23);
    dbg(c);

    //creating bitset of size 5, and initialise with binary string
    bitset <8> d("10011");
    dbg(d);
