//CF EDU - 2 Ptr - Step 2 - Problem F: https://codeforces.com/edu/course/2/lesson/9/2/practice/contest/307093/problem/F
...
struct minmax_stack{
	...
	void push(ll n){
		st.push(n);
		mini.push(::min(mini.top(), n));
		maxi.push(::max(maxi.top(), n));
	}

	ll pop(){
		ll res = st.top();
		st.pop(), mini.pop(), maxi.pop();
		return res;
	}
